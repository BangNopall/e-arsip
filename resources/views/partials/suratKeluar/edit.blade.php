    ...
    <div id="myMaincontent" class="flex-1 flex flex-col overflow-hidden font-Poppins relative">
        ...
        <div class="flex flex-col px-4 py-6">
            <div class="flex items-center justify-between mb-4">
                ...
                <h2 class="text-stone-900 font-medium text-xl tracking-wide">Edit Data Surat Keluar</h2>
            </div>
            <form action="{{ route('dashboard.surat-keluar.update', ['id' => $suratkeluar->id]) }}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="mb-4">
                ...
                <div class="flex flex-col mb-3.5">
                    ...
                    <div
                        ...
                        <input type="date" placeholder="dd/mm/yyyy" id="tanggalSurat" name="tanggal_surat" value="{{ $suratkeluar->tanggal_surat }}"
                ...
                <div class="flex flex-col mb-4">
                    {{-- menampilkan nama dokumen sesuai surat_masuk_id --}}
                    <label class="text-stone-900 text-sm font-medium tracking-wide mb-0.5">Dokumen Surat Ini</label>
                    <div class="flex flex-col text-sm text-stone-900">
                        @foreach ($dokumensurat as $index => $d)
                            <a class="underline" href="{{ asset($d->path) }}">{{ $index + 1 }}. {{ $d->nama_file }}</a>
                        @endforeach
                    </div>
                </div>
